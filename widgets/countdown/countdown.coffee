class Dashing.Countdown extends Dashing.Widget
  ...
  startCountdown: =>
    ...
    end_timestamp = Math.round( Date.parse($(@node).find(".end-time").html())/1000 )
    seconds_until_end = end_timestamp - current_timestamp

    if seconds_until_end < 0
      @set('timeleft', "Times up!")
    else
      ...
      if d > 0
        ...
        if d > 1
          dayname = 'days'
        @set('timeleft', d + " "+dayname)
      else if h > 1
        @set('timeleft', @formatTime(h) + "hrs")
      else if m > 1
        @set('timeleft', @formatTime(m) + "mins")
      else
        @set('timeleft', @formatTime(s) + "secs")
